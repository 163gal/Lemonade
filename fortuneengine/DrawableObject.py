...
class DrawableObject(pygame.sprite.Sprite):
    ...
    def __init__(self, images, textfileName, transparent=False, x=0, y=0):
        ...
        if textfileName != '':
            ...
            while currentLine != '':
                ...
                self.animations[animValues[0]] = [
                    int(animValues[1]), int(animValues[2])]
                currentLine = f.readline()
    ...
    def scale(self, x=None, y=None):
        ...
        for i in range(len(self._images)):
            self._origImages[i] = pygame.transform.scale(
                self._origImages[i], (self.xSize, self.ySize))
    ...
    def rotate(self, angle):
        self.myAngle += angle
        for i in range(len(self._images)):
            self._images[i] = pygame.transform.rotate(
                self._origImages[i], self.myAngle)
    ...
    def calcColorKey(self, x=0, y=0):
        myColorKey = self._images[0].get_at((x, y))
        self.setColorKey(myColorKey)
