...
class Scene(pygame.sprite.RenderUpdates):
    ...
    def __init__(self, sprites):
        ...
        # sprites.getXPos(), sprites.getYPos()])
    ...
    def calcPosition(self):
        ...
        for i in range(len(self._spritelist)):
            if self._spritelist[i][0].getXPos() < lowestX:
                lowestX = self._spritelist[i][0].getXPos()
            if self._spritelist[i][0].getYPos() < lowestY:
                lowestY = self._spritelist[i][0].getYPos()
    ...
    def calcSize(self):
        ...
        for i in range(len(self._spritelist)):
            if (self._spritelist[i][0].getXPos() +
                    self._spritelist[i][0].getXSize()) > highestX:
                highestX = self._spritelist[i][0].getXPos(
                ) + self._spritelist[i][0].getXSize()
            if (self._spritelist[i][0].getYPos() +
                    self._spritelist[i][0].getYSize()) > highestY:
                highestY = self._spritelist[i][0].getYPos(
                ) + self._spritelist[i][0].getYSize()
    ...
    def addObject(self, newDrawableObject):
        ...
        self._spritelist.insert(len(self._spritelist) - 1,
                                [newDrawableObject,
    ...
    def addObjects(self, newDrawableObjects):
        for sprite in newDrawableObjects:
            ...
            self._spritelist.insert(len(self._spritelist) -
                                    1, [sprite, sprite.getXPos(),
                                        sprite.getYPos()])

    def setRelativePositions(self):

        for i in range(len(self._spritelist)):
            self._spritelist[i][1] = self._spritelist[i][0].getXPos() - \
                self.xPos
            self._spritelist[i][2] = self._spritelist[i][0].getYPos() - \
                self.yPos
    ...
    def setPosition(self, newXPos=None, newYPos=None):
        ...
        if newXPos is not None:
            self.xPos = newXPos
        if newYPos is not None:
            self.yPos = newYPos

        for i in range(len(self._spritelist)):

            self._spritelist[i][0].setPosition(
                self.xPos + self._spritelist[i][1],
                self.yPos + self._spritelist[i][2])
    ...
    def scaleScene(self, newXSize=None, newYSize=None):
        ...
        if newXSize is not None:
            xScale = (newXSize * 1.0) / self.xSize
        if newYSize is not None:
            yScale = (newYSize * 1.0) / self.ySize

        for i in range(len(self._spritelist)):
            self._spritelist[i][0].scale(
                xScale * self._spritelist[i][0].getXSize(),
                yScale * self._spritelist[i][0].getYSize())
