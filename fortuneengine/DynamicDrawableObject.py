...
class DynamicDrawableObject(DrawableObject, pygame.sprite.Sprite):
    ...
    def setSpeed(self, xVelocity=None, yVelocity=None):
        ...
        if xVelocity is not None:
            self.xSpeed = xVelocity
        if yVelocity is not None:
    ...
    def update(self, t):
        ...
        if (timePassed) > self._delay:
            if self._frame < self.animations.get(
                    self._current_anim)[0] or \
                        self._frame > self.animations.get(
                    self._current_anim)[1]:
            ...
            if self._frame >= self.animations.get(self._current_anim)[1]:
                self._frame = self._frame % (
                    self.animations.get(self._current_anim)[1])
    ...
    def nextCurrentAnimFrame(self):
        ...
        for cnt in range(len(animations)):
            ...
            if animations[cnt] == self._current_anim:
                if self._frame < self.animations[self._current_anim][
                        0] or self._frame > self.animations[
                        self._current_anim][1]:
                ...
                if self._frame > self.animations[self._current_anim][1]:
                    framesPast = self._frame - \
                        self.animations[self._current_anim][1]
                    self._frame = framesPast - 1 + \
                        self.animations[self._current_anim][0]
